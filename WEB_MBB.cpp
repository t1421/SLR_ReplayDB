...
bool WEB_MBB::FillWebDeck()
{
	return FillWebDeckDeck();
}

bool WEB_MBB::FillWebDeckAction()
{
	MISS;
...
void WEB_MBB::addCard(unsigned short uiCardID, bool Unit, bool Spell, bool Building)
{
	MISS;
...
bool WEB_MBB::getFromCSVBuilding(unsigned short uiCardID)
{
	for (unsigned int j = 0; j < Bro->L->CsvAllCards.size(); j++)
...
bool WEB_MBB::getFromCSVSpell(unsigned short uiCardID)
{
	for (unsigned int j = 0; j < Bro->L->CsvAllCards.size(); j++)
...
bool WEB_MBB::getFromCSVUnit(unsigned short uiCardID)
{
	for (unsigned int j = 0; j < Bro->L->CsvAllCards.size(); j++)
